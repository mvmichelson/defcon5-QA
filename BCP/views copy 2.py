...
from .models import Indicadores_Asig, Log_Revision, SubProceso_V, Control_Cambios, Procedimientos_V
        ...
        if form.is_valid():
            ...
            if aprobado:
                ...
                if not existe:    # Verifica si el PC Vigente existe
                    ...
                    proced_v.servicios_pc.set(proced.servicios_pc.all()) 
                    ...
                    if proced_v.nombre != proced.nombre:
                        print('Cambio Nombre')
                        detalle_log=detalle_log+'Cambio al Nombre '+proced.nombre+', por'+proced_v.nombre+'.'
                        proced_v.nombre = proced.nombre
                        hay_cambios=True

                    if proced_v.tipo != proced.tipo:
                        print('Cambio Tipo')
                        detalle_log=detalle_log+'Cambio al Tipo de PC '+proced.tipo.nombre+', por'+proced_v.tipo.nombre+'.'
                        proced_v.tipo = proced.tipo
                        hay_cambios=True
                    ...
                    if proced_v.escenarios != proced.escenarios:
                        ...
                        detalle_log=detalle_log+'Cambio al Escenario '+proced.escenarios.titulo+', por'+proced_v.escenarios.titulo+'.'
                        proced_v.escenarios = proced.escenarios
                        hay_cambios=True

                    if proced_v.estrategia != proced.estrategia:
                        print('CAmbio en Estrategia')
                        detalle_log=detalle_log+'Cambio a la Estrategia '+proced.estrategia+', por'+proced_v.estrategia+'.'
                        proced_v.estrategia = proced.estrategia
                        hay_cambios=True 

                    if proced_v.resp_proceso != proced.resp_proceso:
                        print('Cambio en el Responsable')
                        detalle_log=detalle_log+'Cambio al Responsable '+proced.resp_proceso+', por'+proced_v.resp_proceso+'.'
                        proced_v.resp_proceso = proced.resp_proceso
                        hay_cambios=True

                    if proced_v.bck_resp != proced.bck_resp:
                        print('Cambio en el Respaldo del Responsable')
                        detalle_log=detalle_log+'Cambio al Respaldo del Responsable '+proced.bck_resp.user_gestor.last_name+', por'+proced_v.bck_resp.user_gestor.last_name+'.'
                        proced_v.bck_resp = proced.bck_resp
                        hay_cambios=True
                    ...
                    if proced_v.bck_ejecutor != proced.bck_ejecutor:
                        ...
                        detalle_log=detalle_log+'Cambio al Respaldo del Ejecutor '+proced.bck_ejecutor.user_gestor.last_name+', por'+proced_v.bck_ejecutor.user_gestor.last_name+'.'
                        proced_v.bck_ejecutor = proced.bck_ejecutor
                        hay_cambios=True
                    ...
                    if proced_v.bck_enlace != proced.bck_enlace:
                        ...
                        detalle_log=detalle_log+'Cambio al Respaldo del Enlace del Comite de Crisis '+proced.bck_enlace.user_gestor.last_name+', por'+proced_v.bck_enlace.user_gestor.last_name+'.'
                        proced_v.bck_enlace = proced.bck_enlace
                        hay_cambios=True

                    
                    # Verifica si se agrego el Servicio
                    for ser_v in proced_v.servicios_pc.all():
                        existe=False
                        for ser_o in proced.servicios_pc.all():
                            if ser_o == ser_v: 
                                    existe=True
                            if not existe_en_serv:
                                detalle_log=detalle_log+'Se agrego el servicio : '+ser_v.nombre+', '

                    # Verifica si se elimino el Servicio
                    for ser_o in proced.servicios_pc.all():
                        existe=False
                        for ser_v in proced_v.servicios_pc.all():
                            if ser_o == ser_v: 
                                    existe=True
                            if not existe_en_serv:
                                detalle_log=detalle_log+'Se elimino  el servicio : '+ser_v.nombre+', '

                    # Verifica si de elimino el Servicio 
                    if ser_o in proced.servicios_pc:
                        print('Cambios en Servicios/Recursos asignados')
                        detalle_log=detalle_log+'Cambio en los Servicios del PC: '

                        for ser, ser_v  in zip(proced.servicios_pc.all(), proced_v.servicios_pc.all()):

                            # Verifica si se agrego el Servicio
                            existe_en_serv=False
                            for ser1 in proced.servicios_pc.all():
                                if ser1.nombre == ser_v.nombre:
                                    existe_en_serv=True
                            if not existe_en_serv:
                                detalle_log=detalle_log+'Se agrego el servicio : '+ser_v.nombre+', '

                            #Verifica se se elimino un Servicio
                            existe_en_ser_v=False
                            for ser1 in proced_v.servicios_pc.all():
                                if ser1.nombre == ser.nombre:
                                    existe_en_serv_v=True
                            if not existe_en_serv_v:
                                detalle_log=detalle_log+'Se elimino el servicio : '+ser_v.nombre+', '
                            
                            # Verifica si se modifico un servicio
                            if ser.nombre != ser_v.nombre:
                                detalle_log=detalle_log + 'Servicio :'+ ser.nombre + 'por:' + ser_v.nombre +','
                            if ser.objetivo != ser_v.objetivo:
                                detalle_log=detalle_log + 'Objetivo : ' + ser.objetivo + 'por:' + ser_v.objetivo +','
                            if ser.contacto != ser_v.contacto:
                                detalle_log=detalle_log + 'Contacto :' + ser.contacto + 'por:' + ser_v.contacto +','
                            if ser.contacto_bck != ser_v.contacto_bck:
                                detalle_log=detalle_log + 'Respaldo de Contacto :' + ser.contacto_bck + 'por:' + ser_v.contacto_bck +','

                        proced_v.servicios_pc.set(proced.servicios_pc.all())
                        hay_cambios=True

                    if proced_v.contactos_pc != proced.contactos_pc:
                        print('Cambios en Contactos')
                        detalle_log=detalle_log+'Cambio en datos de Contactos :'
                        
                        for con, con_v  in zip(proced.contactos_pc.all(), proced_v.contactos_pc.all()):
                            if con.nombre != con_v.nombre:
                                detalle_log=detalle_log + 'Nombre :'+ con.nombre + 'por:' + con_v.nombre +','
                            if con.correo != con_v.correo:
                                detalle_log=detalle_log + 'Correo : ' + con.correo + 'por:' + con_v.correo +','
                            if con.tel_lab != con_v.tel_lab:
                                detalle_log=detalle_log + 'Telefono fijo :' + con.tel_lab + 'por:' + con_v.tel_lab +','
                            if con.cel_lab != con_v.cel_lab:
                                detalle_log=detalle_log + 'Telefono Celular :' + con.cel_lab + 'por:' + con_v.cel_lab +','


                        proced_v.contactos_pc.set(proced.contactos_pc.all())
                        hay_cambios=True

                    if proced_v.pasos != proced.pasos:
                        print('Cambios en Servicios/Recursos asignados')
                        detalle_log=detalle_log+'Cambio en los Pasos del PC '

                        for pas, pas_v  in zip(proced.pasos.all(),  proced_v.pasos.all()):
                            if pas.nro_paso != pas_v.nro_paso:
                                detalle_log=detalle_log + 'Nombre :'+ pas.nro_paso + 'por:' + pas.nro_paso +','
                            if pas.descripcion != pas_v.descripcion:
                                detalle_log=detalle_log + 'Correo : ' + pas.descripcion + 'por:' + pas_v.descripcion +','
                            if pas.ejecutor != pas_v.ejecutor:
                                detalle_log=detalle_log + 'Telefono fijo :' + pas.ejecutor + 'por:' + pas_v.ejecutor +','
                            if pas.tiempo_esp != pas_v.tiempo_esp:
                                detalle_log=detalle_log + 'Telefono Celular :' + pas.tiempo_esp + 'por:' + pas_v.tiempo_esp +','
